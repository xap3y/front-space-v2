...
export function FilesPageClient() {
    ...
    const completedUploadSize = uploadedFiles.reduce((total, file) => total + file.size, 0);
                    ...
                    <div className="space-y-5 p-5 sm:p-6">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-300">Pack link</p>
                                            {isPasswordProtected && <p className="text-[10px] text-amber-400/80">Password protected</p>}
                                        </div>
                                    </div>
                                    <HoverDiv
                                        onClick={() => window.open(getPackUrl(), "_blank", "noopener,noreferrer")}
                                        className="h-8 w-8 !rounded-md !border !p-0"
                                        title="Open pack"
                                        aria-label="Open pack in a new tab"
                                ...
                                <div className="flex gap-2">
                                    <MainStringInput
                                        ...
                                        readOnly
                                        aria-label="Pack URL"
                                        className="min-w-0 flex-1 !rounded-md !border-zinc-800 bg-black/40"
                                        inputClassName="!py-2.5 font-mono !text-xs"
                                    />
                                    <HoverDiv
                                        onClick={() => copyToClipboard(getPackUrl())}
                                        className="h-[42px] w-[42px] shrink-0 !rounded-md !border !p-0"
                                        title="Copy pack URL"
                                        aria-label="Copy pack URL"
                        ...
                        <div>
                            <div className="mb-2 flex items-center justify-between px-1">
                                <p className="text-xs font-medium text-zinc-300">Uploaded files</p>
                                <p className="text-[10px] tabular-nums text-zinc-500">{uploadedFiles.length} {uploadedFiles.length === 1 ? "item" : "items"}</p>
                            </div>
                            <div className="max-h-[290px] space-y-2 overflow-y-auto pr-1">
                                {uploadedFiles.map((file) => (
                                    <div key={file.uniqueId} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-black/25 p-3">
                                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-zinc-800/70 text-emerald-400">
                                        ...
                                        <HoverDiv
                                            ...
                                            className="h-8 w-8 shrink-0 !rounded-md !border !p-0"
                                            title={`Copy URL for ${file.originalFileName}`}
                                            aria-label={`Copy URL for ${file.originalFileName}`}
                                            icon={<FaCopy className="h-3 w-3" />}
                                        />
                                        <HoverDiv
                                            ...
                                            aria-label={`Open ${file.originalFileName} in a new tab`}
                                            icon={<FaExternalLinkAlt className="h-3 w-3" />}
                                        />
                                    </div>
                                ))}

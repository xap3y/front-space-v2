...
export default function TwoFactorClient({apiKey}: {apiKey: string}) {
    ...
    return <div className="box-primary p-5 md:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {!status ? <span className="text-xs text-zinc-500">Loading…</span> : status.enabled ? <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400">Enabled</span> : <HoverDiv type="SAVE" icon={<FaShieldHalved/>} onClick={begin} disabled={busy} className="rounded-lg px-4 py-2 text-sm font-semibold">Enable 2FA</HoverDiv>}
        </div>
        {status?.enabled && <div className="mt-5 flex flex-col gap-3 box-primary p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm text-zinc-300">{status.backupCodesRemaining} backup codes remaining</p><p className="text-xs text-zinc-600">Each recovery code works only once.</p></div><div className="flex gap-2"><HoverDiv type="WARN" icon={<FaKey/>} onClick={() => {setAction("regenerate"); setCode("")}} className="rounded-lg px-3 py-2 text-xs">Regenerate codes</HoverDiv><HoverDiv type="DELETE" icon={<FaXmark/>} onClick={() => {setAction("disable"); setCode("")}} className="rounded-lg px-3 py-2 text-xs">Disable</HoverDiv></div></div>}

        {setup && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-2xl border border-zinc-700 bg-[#101014] p-5 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-lg font-semibold">Set up authenticator</h2><p className="text-xs text-zinc-500">Scan the QR code, then enter the generated code.</p></div><HoverDiv type="INFO" icon={<FaXmark/>} onClick={() => setSetup(null)} className="h-9 w-9 p-0" aria-label="Close"/></div><div className="mt-5 flex flex-col items-center gap-4">{qr && <img src={qr} alt="Authenticator QR code" className="h-52 w-52 rounded-xl bg-white p-2" />}<div className="w-full"><p className="mb-1 text-xs text-zinc-500">Manual setup key</p><HoverDiv type="INFO" icon={<FaCopy/>} onClick={() => {navigator.clipboard.writeText(setup.secret); okToast("Secret copied")}} className="w-full justify-between rounded-lg px-3 py-2 font-mono text-sm tracking-wider"><span className="break-all text-left">{setup.secret}</span></HoverDiv></div><MainStringInput value={code} onChange={(value) => setCode(value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" placeholder="000000" className="w-full rounded-lg border-zinc-700 bg-black/30" inputClassName="px-4 py-3 text-center font-mono text-xl tracking-[.5em]" /><HoverDiv type="SAVE" icon={<FaShieldHalved/>} onClick={confirm} disabled={busy || code.length !== 6} className="w-full rounded-lg py-3 font-semibold">{busy ? "Verifying…" : "Verify and enable"}</HoverDiv></div></div></div>}

        {backupCodes.length > 0 && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-2xl border border-zinc-700 bg-[#101014] p-5"><div className="flex items-center gap-3"><FaKey className="text-amber-400" /><div><h2 className="font-semibold">Save your backup codes</h2><p className="text-xs text-zinc-500">They will not be shown again.</p></div></div><div className="my-5 grid grid-cols-2 gap-2 rounded-xl bg-black/30 p-4 font-mono text-sm">{backupCodes.map(c => <span key={c}>{c}</span>)}</div><div className="flex gap-2"><HoverDiv type="INFO" icon={<FaDownload/>} onClick={download} className="flex-1 rounded-lg py-2 text-sm">Download .txt</HoverDiv><HoverDiv type="SAVE" icon={<FaKey/>} onClick={() => setBackupCodes([])} className="flex-1 rounded-lg py-2 text-sm font-semibold">I saved them</HoverDiv></div></div></div>}

        {action && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"><div className="w-full max-w-sm rounded-2xl border border-zinc-700 bg-[#101014] p-5"><h2 className="font-semibold">{action === "disable" ? "Disable 2FA" : "Regenerate backup codes"}</h2><p className="mt-1 text-xs text-zinc-500">Enter a current authenticator code{action === "disable" ? " or backup code" : ""} to continue.</p><MainStringInput value={code} onChange={(value) => setCode(value.toUpperCase().slice(0, 11))} placeholder="Authentication code" className="mt-4 w-full rounded-lg border-zinc-700 bg-black/30" inputClassName="px-3 py-2 font-mono" /><div className="mt-4 flex justify-end gap-2"><HoverDiv type="INFO" onClick={() => setAction(null)} className="rounded-lg px-3 py-2 text-sm">Cancel</HoverDiv><HoverDiv type={action === "disable" ? "DELETE" : "WARN"} icon={action === "disable" ? <FaXmark/> : <FaKey/>} onClick={finishAction} disabled={busy} className="rounded-lg px-3 py-2 text-sm font-semibold">{busy ? "Working…" : "Continue"}</HoverDiv></div></div></div>}
    </div>;
}
